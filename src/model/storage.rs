use crate::model::{DateMenu, Item, Menu, MenuMeal, RestaurantMenu, Section};
use serde_json::json;
...
impl Storage for DateMenu {
    fn to_json(&self) -> serde_json::Value {
        json!({
            "date": self.date,
            "restaurants": self.restaurants.iter().map(|s| s.to_json()).collect::<Vec<serde_json::Value>>(),
        })
    }
...
mod tests {
    ...
    fn get_test_sections() -> Vec<Section> {
        vec![
            Section {
                name: "Flex Bar".into(),
                items: vec![
    ...
    fn get_test_date_menu() -> DateMenu {
        DateMenu {
    ...
    #[test]
    fn test_date_menu_json() {
        assert_eq!(
            get_test_date_menu().to_json(),
            json!({
                "date": "2021-10-08",
                "restaurants": [
                    {
    ...
    #[test]
    fn test_date_menu_json_min() {
        assert_eq!(
            get_test_date_menu().to_json_min(),
            json!([
                "2021-10-08",
                [
                    [
